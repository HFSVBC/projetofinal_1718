with open('create.sql', 'w') as infile:
    ...
    for i in range(1,16):
        infile.write("CREATE TABLE Semana" + str(i) + " (ID int PRIMARY KEY, IDSala int, IDAluno int, FOREIGN KEY (IDSala) REFERENCES Sala(ID),FOREIGN KEY (IDAluno) REFERENCES Aluno(ID));\n")
